    ...
    <div ref={ref} className={classNames("picker-view", className)} {...rest}>
      ...
      {selectionState === "month" && (
        <MonthTable
          ...
          onChange={(monthValue) => {
            const selectedDate = new Date(yearSelection as number, monthValue, 1);
            setMonth(selectedDate);
            setMonthSelection(monthValue);

            // If maxLevel is month, trigger onChange and don't switch to date view
            if (maxLevel === "month") {
